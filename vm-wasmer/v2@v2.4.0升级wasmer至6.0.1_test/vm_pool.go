...
func newVmPool(contractId *commonPb.Contract, byteCode []byte, log *logger.CMLogger) (*vmPool, error) {
	...
	//module.Exports()
	//serializedBytes, err := module.Serialize()
	...
	//
	//// 4. 保存为新文件
	//err = ioutil.WriteFile("testdata/serialized_module.wasm", serializedBytes, 0644)
	//if err != nil {
	//	panic(err)
	//}
	//
	//fmt.Println("Metered module serialized and saved.")
...
func (p *vmPool) newInstanceFromModule() (*wrappedInstance, error) {
	...
	p.log.Debugf("new instance id: %s", instance.id)
	return instance, nil
}
