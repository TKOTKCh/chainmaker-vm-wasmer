...
func (b *vmBridgeManager) NewWasmInstance(store *wasmer.Store, byteCode []byte) (*wasmer.Instance, error) {
	...
	imports := b.GetImports(store, &env, wasmer.NewImportObject())
...
func (b *vmBridgeManager) GetImports(store *wasmer.Store, env *CMEnvironment, imports *wasmer.ImportObject) *wasmer.ImportObject {
	if imports == nil {
