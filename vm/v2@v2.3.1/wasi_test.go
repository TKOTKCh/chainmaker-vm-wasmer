...
func TestWacsiImpl_CallContract(t *testing.T) {
	...
	context.EXPECT().CallContract(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(&common.ContractResult{}, protocol.ExecOrderTxTypeNormal, common.TxStatusCode_SUCCESS).AnyTimes()
	log := &test.GoLogger{}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, got1, got2, err := w.CallContract(tt.args.requestBody, tt.args.txSimContext, tt.args.memory, tt.args.data, tt.args.gasUsed, tt.args.isLen)
			if (err != nil) != tt.wantErr {
